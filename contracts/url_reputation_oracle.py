...
disagreeing with each other. `safe` used to be taken directly from the
LLM's own claim with no cross-check against `phishing`/`malware`: an LLM
could report `safe: true` while ALSO reporting `phishing: true`, and if
leader and validator both happened to produce that same contradictory
pair, consensus would accept it -- `risk_tier` would correctly compute
"high_risk" from the phishing flag, but `is_safe(url)` would still
return `true`, actively telling a caller a phishing site is safe.
`_parse_and_normalize` now forces `safe` to `false` whenever `phishing`
or `malware` is `true`, regardless of what the LLM claimed, closing that
gap by construction while still letting the LLM mark something unsafe
for other reasons (e.g. scam-like deception) even without either flag
set. The same pattern closes two more pairings:
...
def _parse_and_normalize(raw: typing.Any) -> dict:
    ...
    # `safe` is never trusted at face value when it contradicts the threat
    # flags. An LLM can claim safe=true while ALSO claiming phishing=true
    # or malware=true -- that combination is accepted here at the parsing
    # level (both leader and validator could independently produce the
    # exact same self-contradictory tuple and still reach consensus, since
    # exact-match comparison only checks that fields agree with each
    # other, not that a single response is internally consistent). Forcing
    # safe=false whenever phishing or malware is true closes that gap by
    # construction, while still letting the LLM mark something unsafe for
    # other reasons (scam-like deception) even without either flag set.
    raw_safe = _as_bool(raw, "safe")
    safe = raw_safe and not (phishing or malware)

    # Same guard, wider net: "official" is only meaningful when the site
    # is a real Web3/crypto project (see the prompt's own definition) AND
    # is not itself flagged as a threat. An LLM claiming official=true
    # while also reporting category="Unknown", phishing=true, or
    # malware=true is self-contradictory in the same way as
    # safe=true + phishing=true -- a phishing page is by definition
    # impersonating something else, so it cannot genuinely be the
    # official site, and would pass consensus just as easily if leader
    # and validator both produced the same contradictory pair. Force it
    # false here rather than trust it.
    raw_official = _as_bool(raw, "official")
    official = raw_official and category != "Unknown" and not (phishing or malware)
    ...
    # trust_score like 90 -- technically harmless for consensus/decisions
    # (every consumer is told to read risk_tier, never trust_score), but
    # still a visibly self-contradictory number next to a "high_risk"
    # label for anyone just glancing at the UI. Clamping it into the
    # high_risk range whenever a threat flag is set keeps the displayed
    # number consistent with the bucket it's forced into.
