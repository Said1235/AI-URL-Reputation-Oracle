...
re-runs `leader_fn` (a fresh download + a fresh LLM call) and compares:
    - The boolean decision fields (phishing, malware, gambling, adult,
      official) must match exactly.
    ...
    - `safe` is not compared directly -- it's derived from `risk_tier`
      (`safe = risk_tier == "trusted"`), so it's automatically consistent
      once `risk_tier` matches. See below for why it's derived rather
      than requested from the LLM at all.
...
on `official`/`risk_tier` only reaches agreement if every LLM provider
interprets those fields the same way. An early version of `_build_prompt`
defined `official` only in terms of Web3 projects and never precisely
defined the (then-requested) `safe` field at all, which was fine for
on-topic URLs but caused persistent MAJORITY_DISAGREE / UNDETERMINED
results for ordinary non-Web3 sites (e.g. a large adult-content site with
no phishing or malware): different providers guessed differently at
whether "official" applies to something outside the Web3 category, and a
borderline `trust_score` near a tier boundary (e.g. 29 vs 30) made it
worse. The current prompt fixes this by giving every requested field a
precise, provider-independent definition -- notably, `trust_score` is
defined as a pure security/scam-risk judgment, decoupled from content
type or Web3-relevance, and `official` is explicitly false by default
for anything outside the fixed Web3 category list.

Exact-match consensus also has to guard against a single response being
internally self-contradictory, not just against leader/validator
disagreeing with each other. `safe` is the clearest example: it used to
be a field the LLM reported directly, only forced to `false` when
`phishing` or `malware` was `true`. That closed one direction of the
contradiction but not the other -- with both flags `false`, an LLM could
still pair `safe: true` with a low score (stored as
`risk_tier: "high_risk"`) or `safe: false` with a high score (stored as
`risk_tier: "trusted"`), and either self-contradictory tuple would pass
consensus if leader and validator both produced it, since exact-match
comparison only checks that the two responses agree with each other, not
that either one is internally consistent. `safe` is no longer requested
from the LLM at all (removed from the prompt's JSON schema) and is
instead derived directly from `risk_tier` in `_parse_and_normalize`:
`safe = (risk_tier == "trusted")`. That makes both directions of the
contradiction impossible by construction instead of guarding against
only the phishing/malware-shaped half of it.

The same "derive, don't trust" pattern closes two more pairings:
    - `official`: only meaningful for a real Web3/crypto category, and
      inherently incompatible with `phishing` (a phishing page is by
      definition impersonating something else, so it cannot genuinely be
      the official site). Forced to `false` whenever `category` is
      `"Unknown"` or `phishing`/`malware` is `true`.
    - `trust_score`: informational only and never part of the consensus
      gate, but nothing stopped an LLM from pairing `phishing: true` with
      a high score like 90 -- harmless for decisions (every consumer is
      told to read `risk_tier`, never `trust_score`), but a visibly
      self-contradictory number next to a "high_risk" label for anyone
      glancing at the stored report. Clamped into the high_risk numeric
      range whenever `phishing` or `malware` is `true`.
"""
...
- "trust_score": 0-100, a SECURITY-RISK score ONLY -- do not base it on
  content type, legality, or how "wholesome" the site is. A
  well-established, non-malicious site should score high (70-100)
  regardless of its topic or whether it's related to Web3, even if it is
  gambling or adult content (that's already captured separately by the
  "gambling"/"adult" flags). Only lower the score for concrete signs of
  risk: phishing, malware, deceptive practices, or scam-like behavior.
...
def _parse_and_normalize(raw: typing.Any) -> dict:
    ...
    # Closes the last gap in the same class of bug: trust_score is
    # informational only and never gates consensus (risk_tier does), but
    # nothing stopped an LLM from pairing phishing=true with a high
    # trust_score like 90 -- harmless for decisions (every consumer is
    # told to read risk_tier, never trust_score), but a visibly
    # self-contradictory number next to a "high_risk" label for anyone
    # glancing at the stored report. Clamped into the high_risk numeric
    # range whenever a threat flag is set, before risk_tier is computed
    # from it below.
    if phishing or malware:
        trust_score = min(trust_score, RISK_TIER_HIGH_RISK_MAX)

    # risk_tier is the single source of truth every other "is this
    # dangerous" signal is derived from below -- compute it once, from
    # the now-finalized phishing/malware/trust_score.
    risk_tier = _compute_risk_tier({"phishing": phishing, "malware": malware, "trust_score": trust_score})

    # `safe` is no longer read from the LLM's response at all (the prompt
    # doesn't ask for it -- see _build_prompt). It's fully derived from
    # risk_tier instead: true iff risk_tier == "trusted".
    #
    # A previous version took `safe` from the LLM's own claim and only
    # forced it false when phishing/malware was true. That closed one
    # direction of the contradiction but not the other: with phishing and
    # malware both false, an LLM could still pair safe=true with a low
    # score (stored as risk_tier="high_risk") or safe=false with a high
    # score (stored as risk_tier="trusted") -- either self-contradictory
    # tuple would pass consensus if leader and validator both produced
    # it, since exact-match comparison only checks that the two responses
    # agree with each other, not that either one is internally
    # consistent. Deriving `safe` directly from `risk_tier` makes both
    # directions of the contradiction impossible by construction, instead
    # of guarding against only the phishing/malware-shaped half of it.
    safe = risk_tier == "trusted"

    # Same guard, tied directly to the same source of truth: "official"
    # is only meaningful when the site is a real Web3/crypto project (see
    # the prompt's own definition) AND the overall risk decision is
    # "trusted". `risk_tier == "trusted"` already implies phishing/malware
    # are both false (by construction of _compute_risk_tier above), so
    # this single condition subsumes the old separate phishing/malware
    # check while also closing one more gap: the prompt's own trust_score
    # definition ties a low score to "phishing, malware, deceptive
    # practices, or scam-like behavior", so official=true paired with a
    # low score (risk_tier="high_risk" or "caution") even with both flags
    # false is just as self-contradictory as official=true + phishing=true
    # -- and would pass consensus just as easily if leader and validator
    # both produced the same contradictory pair. Force it false here
    # rather than trust it.
    raw_official = _as_bool(raw, "official")
    official = raw_official and category != "Unknown" and risk_tier == "trusted"
...
def _compare_analysis(leader_data: dict, validator_data: dict) -> bool:
    ...
    `safe` is intentionally NOT in this list even though it's a decision
    field: it's fully derived from `risk_tier` (`safe = risk_tier ==
    "trusted"`, computed identically for leader and validator in
    `_parse_and_normalize`), so once `risk_tier` matches, `safe` is
    mathematically guaranteed to match too -- comparing it separately
    would be redundant, not an extra safety check.
    """
    decision_fields = ("phishing", "malware", "gambling", "adult", "official", "category", "risk_tier")
